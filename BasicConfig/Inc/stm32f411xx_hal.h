 ...
 *      Author: JUAN ESTEBAN
 *      En este archivo estan descritos toidos los perisfericos que componen
 *      al microcontrolador.
 */
...
#define HSI Clock_SPEED  16000000 // Value for the main clock signal (frequency) (HSI--> High Speed Internal)
#define HSE Clock_SPEED  4000000 // Value for the main clock signal (frequency) (HSE--> High Speed External)

#define NOP()     asm("NOP")
#define _weak    _attribute_((weak))

/*
 * Base addresses pf flash and SRAM memories
...
#define SRAM_BASE_ADDR   0x3000000U   // Esta es la memoria RAM, 128KB
...
/**
 * AHBx and APBx bus Peripherals ase addresses
 */
 ...
 * perisfericos descritos en la Tabla 1 del manual de referencia del micro.
 * Observe que en dicha tabla esta a su vez dividida en cuatro segmentos, cada uno correspondiente a
 * APB1, APB2, AHB1, AHB2.
 *
 * Comenzar de arriba hacia abajo como se muestra en la tabla. Inicia USB_OTG_FS (AHB2)
 *
 * Nota: Solo lo vamos a  ahcer con los elementos que necesitamos en este ejercicico; RCC y GPIOx
 */

//Posiciones de memoria para perisfericos del AHB2
#define USB_OTG_FS_BASE_ADDR (AHB2_BASE_ADDR + 0x0000U)

/*
 * Posiciones de memoria para perisfericos del AHB1
...
/*++++======== INICIO de la descripcion de los elementos q	ue componen el perisferico =======+++++
 *
 ...
 * 6.3.1 (RCC_CR) hasta el 6.3.12 (RCC_APB2ENR), 6.3.17 (RCC_BDCR) y 6.3.18 (RCC_C5R)
 *
 * NOTA: La posicion de memoria (offset) febe encajar perfectamente con la posicicon de meoira indicada
 * en la hoja de datos del equipo. Observbe que los elementos  "reservedc" tambien estan presentes alli.
 *
 */
...
typedef struct
{
	...
	volatile uint32_t CIR;       //Clock interrupt reset register         ADDR_OFFSET:  0x0C
	...
	volatile uint32_t reserbed0; // reserved                              ADDR_OFFSET:  0x18
...
/*
 *
 * Como se vio en la clase de intro a las estructuras, hacemos un puntero a RCC_RefDef_t que apunta a la posicion exacta
 * del perisferico RCC, de forma que cada miembro de la estructura coincide
 * con cada uno de los SFR en la memoria del MCU. Esta accion la estamos haciendo en un MACRO, de forma que el nuevoi elemento
 * "RCC" queda disponible para cada clase .C (archivo .c) que incluya este archivo
 */

#define RCC  ((RCC_RegDef_t *)RCC_BASE_ADDR)

/*Descripcion bit a bit de cadfa uno de los registros del que componen al periferico RCC
 * En este primer ejercicio muchos de ellos no son necesarios, se dejan sin implementar
 * No son necesarios para el primer ejercicio:
 * Pero si implementaremos los siguientes para el ejercicio:
 *
 * 6.3.10 RCC_AHB1ZENR
 * 6.3.11 RCC_APB1ENR
 * 6.3.12 RCC_APB2ENR
 ...
 * Definicion de la estructura de datos que representa a cada uno de los registros que componen el
 * periferico GPIO.
 * Debido a que el periferico GPIOc es muy simple, no es muy necesario crear la descripcion bit a bit
 * de cada uno de los registros que componen el dicho periferico que si es necesario comprender que
...
typedef struct
{
	...
	volatile uint32_t AFRL;        // alternative function low register     ADDR_OFFSET:  0x20
	volatile uint32_t AFRH;        // alternative function high register    ADDR_OFFSET:  0x24

} GPIOx_RegDef_t;

/*
 * Al igual que cpon el RCC, Creamos un puntero a la estructura que define a GPIOx y debemos hacer
